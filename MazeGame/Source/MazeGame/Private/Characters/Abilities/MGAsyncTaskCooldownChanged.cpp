...
#include "Characters/Abilities/MGAsyncTaskCooldownChanged.h"

UMGAsyncTaskCooldownChanged * UMGAsyncTaskCooldownChanged::ListenForCooldownChange(UAbilitySystemComponent * AbilitySystemComponent, FGameplayTagContainer InCooldownTags, bool InUseServerCooldown)
{
	UMGAsyncTaskCooldownChanged* ListenForCooldownChange = NewObject<UMGAsyncTaskCooldownChanged>();
	...
	AbilitySystemComponent->OnActiveGameplayEffectAddedDelegateToSelf.AddUObject(ListenForCooldownChange, &UMGAsyncTaskCooldownChanged::OnActiveGameplayEffectAddedCallback);
	...
	{
		AbilitySystemComponent->RegisterGameplayTagEvent(CooldownTag, EGameplayTagEventType::NewOrRemoved).AddUObject(ListenForCooldownChange, &UMGAsyncTaskCooldownChanged::CooldownTagChanged);
	}

	return ListenForCooldownChange;
}

void UMGAsyncTaskCooldownChanged::EndTask()
{
	if (IsValid(ASC))
	{
...
void UMGAsyncTaskCooldownChanged::OnActiveGameplayEffectAddedCallback(UAbilitySystemComponent * Target, const FGameplayEffectSpec & SpecApplied, FActiveGameplayEffectHandle ActiveHandle)
{
...
void UMGAsyncTaskCooldownChanged::CooldownTagChanged(const FGameplayTag CooldownTag, int32 NewCount)
{
	if (NewCount == 0)
	{
...
bool UMGAsyncTaskCooldownChanged::GetCooldownRemainingForTag(FGameplayTagContainer InCooldownTags, float & TimeRemaining, float & CooldownDuration)
{
